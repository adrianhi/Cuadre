...
function validateTarget(
  value: z.infer<typeof ruleFields>,
  ctx: z.RefinementCtx,
) {
  const length = value.pattern
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/\s+/g, ' ')
    .trim().length;
  if (value.matchType === 'CONTAINS' && (length < 2 || length > 60)) {
    ctx.addIssue({
      ...
      path: ['pattern'],
      message: 'El patrón debe tener entre 2 y 60 caracteres',
    });
  }
  if (value.matchType === 'MERCHANT' && !value.merchantKey) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['merchantKey'],
      message: 'Selecciona un comercio',
    });
  }
}

export const createCategoryRuleInputSchema = ruleFields.superRefine(validateTarget);
export const updateCategoryRuleInputSchema = ruleFields
  .partial()
...
export const ruleMerchantSchema = z.object({
  key: z.string(),
  label: z.string(),
});
