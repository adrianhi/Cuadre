...
export const recurringService = {
  async radar(
    currency: string,
    signal?: AbortSignal,
  ): Promise<RecurringRadarDto> {
    const response = await httpClient.get("/recurring", {
      params: { currency, window: 30 },
      signal,
    });
  ...
  async linkTransaction(
    ...
    const response = await httpClient.post(
      `/recurring/${recurringBillId}/link-transaction`,
      { transactionId },
    );
    return parseResponse(linkRecurringTransactionResponseSchema, response.data)
      .data;
  },
  async unlinkTransaction(
    recurringBillId: string,
    transactionId?: string,
  ): Promise<{ unlinked: boolean; recurringBillId: string }> {
    const response = await httpClient.post(
      `/recurring/${recurringBillId}/unlink-transaction`,
      { transactionId },
    );
